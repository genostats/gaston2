...
class SNPmatrix {
  ...
  std::array<int, 4> compute_stats(int Indnb) {
    int total = SNPs.size();
    if (total == 0) throw std::out_of_range("No SNPs loaded into the SNPMatrix !");
    if (Indnb > SNPs[0]->nbInds() - 1) throw std::out_of_range("You're trying to access an out-of bound indiviual.");
    std::array<int, 4> stats = {{0, 0, 0, 0}};
    for (int i = 0; i < total; i++) { //parcourt tous les SNPs
      auto vec = SNPs[i];
      int j = 0;
      for(auto pa = vec->begin(); pa != vec->end(); ++pa) { // parcourt le SNP[i]
        if (j == Indnb) {
          stats[(int)*pa]++;
         }
        j++;
