 ...
 public:
  ...
  size_t size() const {
    switch (type_) {
      ...
      case BOOL: {
        return ((std::vector<bool>*)handler.get())->size();
      }
      default:
  ...
  int handler_use_count() const {
    return handler.use_count();
  }
#endif
  ...
  Column(datatype ty) : type_(ty) {
    switch (ty) {
      ...
      case BOOL: {
        std::vector<bool> vec;
        handler = std::make_shared<std::vector<bool>>(vec);
        break;
      }
      default:
  ...
  Column(const Column col, const intVec& keep) {
    ...
    switch (ty) {
      ...
      case BOOL: {
        *this = ColumnExtract<bool>(col, keep);
        break;
      }
      default:
 ...
 public:
  ...
  Column(const Column col1, const Column col2) {
    ...
    switch (type_) {
      ...
      case BOOL: {
        *this = ColumnAppend<bool>(col1, col2);
        break;
      }
      default:
 ...
 public:
  ...
  void push_back_convert(const std::string& x) {
    switch (type_) {
      ...
      case BOOL: {
        // TODO : cette fonction doit pouvoir prendre du R ou du C++ ?
        // TODO : réfléchir à ce que peux contenir la chaine de caractères
        // maybe use un if(islower()) d'abord ? 
        std::cout << "For push_back_convert with bool using stob() \n";
        ((std::vector<bool>*)handler.get())->push_back(stob(x));
        break;
      }
      default:
  ...
  void push_back_convert(const char* x) {
    switch (type_) {
      ...
      case BOOL: {
        // TODO : tout pareil qu'au-dessus !
        std::string s(x);
        std::cout << "For now push_back_convert with bool from char * \n";
        ((std::vector<bool>*)handler.get())->push_back(stob(s));
        break;
      }
      default:
  ...
  char* push_back_token(const char* x) {
    ...
    switch (type_) {
      ...
      case BOOL: {
        // TODO : encore une fois, même chose qu'avant
        std::cout << "For now push_back_token with bool push une str\n";
        ((std::vector<bool>*)handler.get())->push_back(strtob(x, &end));
        break;
      }
      default:
        throw std::runtime_error("Can't convert char * to type NONE");
